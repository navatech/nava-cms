				...
				<div class="table-responsive">
					<table class="table m-t-50">
						...
						<tr>
							...
							<td class="text-right"><?= number_format($order_item->price*$order_item->quantity)?> ₫</td>
						</tr>
						<?php endforeach; ?>
						</tbody>
					</table>
				...
				<div class="container-sm-height">
					<div class="row row-sm-height b-a b-grey">
						...
						<div class="col-sm-5 col-sm-height col-middle clearfix sm-p-b-15">
							...
							<h3 class="no-margin"><?= number_format($model->getDiscount($total,$model->discount));?> ₫</h3>
						</div>
						<div class="col-sm-5 text-right bg-black col-sm-height padding-15">
							<h5 class="font-montserrat all-caps small no-margin hint-text text-white bold"><?= Translate::total_price()?> ₫</h5>
							<h1 class="no-margin text-white"><?= number_format($total-$model->getDiscount($total,$model->discount)); ?> ₫</h1>
						</div>
					</div>
				</div>
			</div>
